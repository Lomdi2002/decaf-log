...
日々のカフェイン摂取量を記録し、1日のカフェイン目標に対する進捗・連続達成日数・日別の推移を確認できるWebアプリ（Version 1.6）。

飲み物・カフェイン量・摂取日時を記録し、今日の合計摂取量と履歴を確認できます。また、1日のカフェイン目標値を設定し、ダッシュボードで目標に対する進捗と連続達成日数を、履歴画面で直近7日間の日別摂取量グラフを確認できます。記録画面ではよく飲む飲み物をプリセットから選んで手早く記録でき、履歴画面から既存の記録を編集することもできます。
...
- `/record`への飲み物プリセット（chip形式のボタン）の追加：コーヒー(90mg)・緑茶(30mg)・紅茶(30mg)・烏龍茶(20mg)・エナジードリンク(100mg)・コーラ(35mg)の6種類
- プリセットは「飲み物」「カフェイン量」へ値をセットするだけの入力ショートカット（選択中状態やハイライトは持たない）
- 選択後も「飲み物」「カフェイン量」を自由に手動編集可能
- 「※カフェイン量は目安です。選択後に変更できます。」の注意書きを表示
- プリセットを使わない従来通りの手入力フローも維持

### Version 1.6

- `/history` の各記録への「編集」リンク追加、`/records/:id/edit` でのカフェイン記録編集機能
- 編集可能な項目は「飲み物名」「カフェイン量」「摂取日時」の3つのみ（登録日時は変更不可）
- 保存成功時は `/history` へ遷移し、履歴・ダッシュボード・日別グラフ・連続達成日数へ自動的に反映される
- キャンセル時は変更を保存せず `/history` へ戻る
- 該当記録が存在しない場合（削除済み・不正なIDなど）は、通信エラーとは区別した「指定された記録が見つかりませんでした。」を表示

## 未実装機能（Version 1.7以降で検討）
...
- `caffeine_records`：anonロールへ `SELECT` / `INSERT` / `DELETE` に加え、`UPDATE` を `drink_name` / `caffeine_mg` / `consumed_at` の3カラムに限定したカラムレベルGRANTとして許可（Version 1.6）。`id` / `created_at` はUPDATEできない
...
**これはMVP・デモ用途に限定した暫定的な構成です。** 特に `caffeine_records` は認証なしでanonロールに書き込み・削除・（3カラム限定の）更新まで許可しているため、
公開URL（および anon key）を知る誰でもデータを追加・削除・書き換えできる状態になります。個人利用やデモの範囲では許容していますが、
...
**Version 1.5の飲み物プリセットに関する既知の制約：** プリセットのカフェイン量は正確な固定値ではなく目安値です。商品・容量・抽出方法等によって
実際のカフェイン量は異なります。プリセットはコーヒー・緑茶・紅茶・烏龍茶・エナジードリンク・コーラの6種類に固定されており、ユーザーが独自の
プリセットを追加・編集する機能はありません。プリセットの定義はDBではなくフロントエンド（`src/lib/drinkPresets.js`）の定数として保持しています。

**Version 1.6の記録編集機能に関する既知の制約：** 認証機構がないまま`caffeine_records`へのUPDATE権限をanonロールへ許可しているため、
公開URLとanon keyを知る誰でも既存の記録を書き換えられる状態になります。UPDATE可能なのは`drink_name`・`caffeine_mg`・`consumed_at`の
3カラムのみで、`id`・`created_at`はUPDATEできません。削除と異なり、書き換え（改ざん）はユーザー本人が気づきにくいという性質があります。
一般公開サービスとして提供する場合は、上記のSupabase Auth + `user_id`単位のRLSへの移行が必要です。また、編集履歴・監査ログ
（誰がいつどの値からどの値へ変更したか、変更前の値の確認）は実装していません。
...
├── pages/        # 画面単位のコンポーネント（/, /record, /history, /records/:id/edit, /settings）
